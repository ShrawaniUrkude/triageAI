...
mkdir nexora-ai
cd nexora-ai
git clone https://github.com/dev-rathankumar/nexora-ai-public.git .
```
...
> **Note:** This project runs on a local **Ollama** model — no API key needed.
> Install [Ollama](https://ollama.com), pull a model (`ollama pull mistral:7b`),
> and keep the Ollama app running while the server is up.
...
|----------|----------|------|
| sanketsd | sanketsd | Customer |
| fraud_test | sanketsd | Test fraud detection |
...
Make sure the Ollama app is running and the model in `OLLAMA_MODEL` is pulled (`ollama list`).
